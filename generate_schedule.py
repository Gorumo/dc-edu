...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .empty{color:#4a505c}
  .audrow{display:flex;gap:14px;padding:9px 15px;background:#13161c;border-bottom:1px solid var(--line);font-size:11.5px;flex-wrap:wrap}
  .aud{color:var(--muted)}.aud b{color:#ffd9a8;font-weight:600}
...
<header>
  ...
  <div class="legend">
    ...
    <span class="lg"><span class="dot" style="background:var(--nit)"></span>ИИ-м: НиТ — Наука и технологии</span>
...
function cell(s){
  ...
  return `<span class="cell"><span class="bar" style="background:${discColor(s)}"></span>`+
    `<span><span class="disc">${s}</span>${tea}</span></span>`;
}
  ...
  DATA.forEach(m=>{
    ...
    grid.insertAdjacentHTML("beforeend",`
      <div class="mod">
        ...
        <div class="audrow">
          <span class="aud">Поток 1: <b>ауд. ${m.aud1}</b></span>
          <span class="aud">Поток 2: <b>ауд. ${m.aud2}</b></span>
        </div>
        <table>
